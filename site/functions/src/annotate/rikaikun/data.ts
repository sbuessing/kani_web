...
class RcxDict {
  ...
  async init() {
    ...
    [, , this.nameDict, this.nameIndex] = await Promise.all([
      ...
      this.fileReadAsync(PATH + 'data/names.dat'),
      this.fileReadAsync(PATH + 'data/names.idx'),
    ]);
  ...
  loadNames() {
    ...
    this.nameDict = this.fileRead(PATH + 'data/names.dat');
    this.nameIndex = this.fileRead(PATH + 'data/names.idx');
  }
  ...
  async loadDictionaries(): Promise<void> {
    [this.wordDict, this.wordIndex, this.kanjiData, this.radData] =
      await Promise.all([
        this.fileReadAsync(PATH + 'data/dict.dat'),
        this.fileReadAsync(PATH + 'data/dict.idx'),
        this.fileReadAsync(PATH + 'data/kanji.dat'),
        this.fileReadAsyncAsArray(PATH + 'data/radicals.dat'),
      ]);
  }

  async loadDeinflectionData() {
    const buffer = await this.fileReadAsyncAsArray(PATH + 'data/deinflect.dat');
    let currentLength = -1;
